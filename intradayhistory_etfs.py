...
# DB + tickers
# ----------------------------
def get_active_etfs(conn):
    with conn.cursor() as cur:
        cur.execute(
            """
            SELECT ticker
            FROM tickers_info
            WHERE active = true AND type = 'ETF' AND trading_universe = true
            ORDER BY ticker
            """
        )
...
def fetch_and_store_etfs(**context):
    if not API_KEY:
    ...
    lookup_conn = psycopg2.connect(**DB_CONFIG)
    try:
        tickers = get_active_etfs(lookup_conn)
    finally:
        lookup_conn.close()
...
# ----------------------------
with DAG(
    dag_id="fetch_etfs_historical_data",
    default_args=default_args,
    description="Fetch 10-min bars for active ETFs from tickers_info (paged, upsert, skip already-loaded chunks)",
    ...
    task_fetch_store = PythonOperator(
        task_id="fetch_and_store_etfs_bars",
        python_callable=fetch_and_store_etfs,
    )
