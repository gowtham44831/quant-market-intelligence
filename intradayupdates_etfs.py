...
def get_active_etfs(conn):
    with conn.cursor() as cur:
        cur.execute(
            ...
            SELECT ticker
            FROM tickers_info
            WHERE type = 'ETF' AND trading_universe = true
            ORDER BY ticker
            """
        )
...
def fetch_intraday_etfs(**kwargs):
    if not API_KEY:
...
dag = DAG(
    dag_id="intraday_etfs_10min",
    start_date=datetime(2025, 12, 18, tzinfo=local_tz),
    schedule="*/10 8-16 * * 1-5",
    catchup=False,
    max_active_runs=1,
    tags=["production-support", "risk-data", "intraday", "etf"],
)

PythonOperator(
    task_id="fetch_intraday_etfs",
    python_callable=fetch_intraday_etfs,
    dag=dag,
)
